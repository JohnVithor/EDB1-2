	 	...
	 	switch(c) {
			...
			case 'q':
				if (t_flag == 1) {
					...
					if (amount_tests <= 0 or amount_tests >= 10000) {
			...
			case 'r':
					...
					if (range < 50 or range >= 1000000000) {
...
//	Como é mantida uma cópia da organização do vetor é necessário dividir o valor por 2
					...
					break;
			case '?':
				if (optopt == 'f' or optopt == 'q' or optopt == 'r') {
	...
	if ( (a_flag == 0 and func == -1) or (v_flag == 0 and i_flag == 0) ) {
...
//						Vector organizado com elementos em ordem aleatória
		...
		std::string func_names[] = { "Insertion sort","Selection sort","Bubble sort","Merge sort",
